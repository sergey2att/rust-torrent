...
Generated: 2026-09-06T14:53:01.724Z
...
Прожарка этапа 5 (DHT/Kademlia BEP 5, magnet, extension protocol BEP 10 + ut_metadata BEP 9) завершена. Рамка: максимально промышленное решение без ущерба производительности — полные k-buckets, полный DHT-responder, Stream как публичный контракт dht, magnet-сценарий целиком внутри engine с фазой metainfo в той же сессии. Fail-closed верификация метаданных по SHA-1 внутри fetch_metadata. Все 13 вопросов resolved.
...
- Routing table: полные k-buckets (k=8), промышленная Kademlia-таблица, не плоский кэш. Периодический refresh/републикация — вне рамок этапа.
- Публичный API dht: find_peers -> impl futures::Stream (dep futures-core, без рантайма); внутри DhtClient — cloneable хэндл-актор с одним UdpSocket и mpsc-командами.
- DHT responder: полный набор — отвечаем на ping, find_node, get_peers (token + peers из локального кэша/nodes), announce_peer (валидация token, пополнение кэша). Активный refresh — нет.
- Magnet-сценарий полностью внутри engine: session() принимает Source::Torrent(Torrent) | Source::Magnet(MagnetLink).
- Фаза metainfo в той же сессии: hub стартует без PieceManager/storage; наш битфилд не шлётся до получения верифицированных метаданных; после — построение Torrent, создание PieceManager+storage, переинициализация живых peer-задач; пиры без ext-бита отключаются сразу.
- ext-metadata: SHA-1 проверка внутри fetch_metadata (fail-closed, HashMismatch — байты наружу не выходят); куски 16 КиБ последовательно в соединении; engine гонит все ext-пиры параллельно, первый полный info побеждает; metadata_size 0 или > 4 МиБ -> disconnect.
- Порт DHT: UDP биндится на тот же номер, что TCP-слушатель пиров (cli 6881, тесты 0).
- parse_magnet_uri: hex-40 + base32-32; btmh/v2 -> UnsupportedMagnet; несколько btih-xt -> ошибка; dn -> display_name (логи); tr -> trackers в порядке следования.
- Источники пиров при magnet: DHT + трекеры из tr= параллельно (дедуп адресов в хабе); приёмочный smoke — magnet без tr= (чистый DHT).
- Константы Kademlia: node_id — случайные 20 байт per-session; k=8; α=3; KRPC timeout 2 с, 1 попытка в обходе (bootstrap-пинг 3 ретрая); token TTL 5 мин per-node; tx-id 2 байта; ответ nodes — до 8 ближайших, принимаем ≤16; локальный кэш пиров 256/info_hash LRU; announce_peer без валидного token -> error 203.
- Отдача метаданных: отвечаем на чужие ut_metadata request — data при наличии верифицированных метаданных, reject иначе; disconnect не делаем.
- Ext handshake: {"m":{"ut_metadata":1},"v":"RT 1.0"} + metadata_size после сбора; чужие поля игнорируем кроме m.ut_metadata и metadata_size; входящие маршрутизируем по объявленному пиром локальному id (ловушка: id не фиксирован).
- CLI: аргумент — .torrent или magnet (автоопределение по префиксу); HubEvent::Metadata (имя+размер) для лога/прогресса; до метаданных показываем поиск метаданных + число найденных пиров.
- Тест-план: KRPC-юниты на зафиксированных bencoded-байтах (эталон codec.rs); mock DHT-узлы на голом UdpSocket; magnet-юниты (hex/base32/ошибки); ut_metadata с фейковым пиром (обрывы на суффиксах, кривой хэш, metadata_size DoS); engine-интеграция с magnet-сессией против фейкового роя (включая регресс: битые метаданные от одного пира -> берём у другого); #[ignore] smoke: ping router.bittorrent.com, get_peers Ubuntu-хэша, полный magnet-цикл без трекеров и .torrent.

## Open Risks

- DHT-рой публичных bootstrap-узлов может быть нестабилен из трёх узлов — smoke-тесты network-dependent (учитывать при CI).
- Фаза metainfo — самый крупный рефакторинг session.rs с этапа 3: переинициализация peer-задач под новую геометрию кусков требует аккуратности (риск рассинхрона — урок этапа 3 про отмену чтений).
- Периодический refresh таблицы отложен: после долгого простоя таблица stale — для наших сценариев (single-torrent) приемлемо, отметить при полировке (этап 6).

## Next Decision Needed

Старт реализации: крейт dht (KRPC, routing table, актор, responder), крейт ext-metadata, magnet в metainfo, интеграция в engine + cli.
